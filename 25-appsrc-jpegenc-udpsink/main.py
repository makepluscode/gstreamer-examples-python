...
def create_pipepline(pipeline: Gst.Pipeline):
    ...
    pay = Gst.ElementFactory.make("rtpjpegpay", "pay")
    ...
    caps2 = Gst.Caps.from_string("video/x-raw, format=YUY2")
    ret = ret and convert.link_filtered(overlay, caps2)
    ret = ret and overlay.link(encode)
    ret = ret and encode.link(pay)
    ret = ret and pay.link(sink)
